...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec3 view_direction, Material material){
	...
	*/
	...
	vector<Light *>::iterator iter = lights.begin();
	...
	for(auto & light : lights) {
		...
		float diffuse = glm::dot(direction, normal);
		...
		glm::vec3 half_vector = glm:: normalize(direction + view_direction);


		// Specular

		float specular = glm::pow(glm::dot(half_vector, normal), 4 * material.shininess);
		...
		color = color + light->color * ((diffuse * material.diffuse) + (specular * material.specular));
...
int main(int argc, const char * argv[]) {
	...
	if (argc > 2) {
		// moving the angle around the first light source 
