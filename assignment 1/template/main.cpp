...
public:
    ...
    Hit intersect(Ray ray){
		...
		Hit hit;
		hit.hit = false; // because initially we assume the ray isn't hitting anything
		...
		glm::vec3 c_rel = center - ray.origin;

		//find the adjacent side a to the triangle intersection
		// a = <c, d>
		float a = glm::dot(c_rel, ray.direction); 

		//90 degree angle distance from sphere center to ray
		//D = sqrt(||c||^2 - <c, d>^2)
		float D = sqrt(glm::dot(c_rel, c_rel) - a*a); 
		
		//result, closest solution
		float t_result; //variable t already exists in code
		//holders
		float t1, t2;

		//The order of the condition matters:
		//1. no rays touching
		//2. must be the intersection
		// (won't work as 3rd condition as when setting another sphere)
		//3. then we check the rest


		// if D > r -> no solution
		if (D > radius) {
			return hit;
		} else if (D < radius) { //if D < r -> two solutions
			
			t1 = a - sqrt((radius*radius) - (D*D));
			t2 = a + sqrt((radius*radius) - (D*D));
		...
		} else { //if D=r -> one solution
			// t = <c, d> +- sqrt(r^2 - D^2)
			t_result = a + sqrt((radius*radius) - (D*D));
    ...
    for(int i = 0; i < width ; i++)
        for(int j = 0; j < height ; j++){
			...
			*/
			...
			//therefore the direction will be
