...
private:
	/// Local normal of the plane
	glm::vec3 normal = glm::vec3(0.0f, 1.0f, 0.0f);
	/// Local point on the plane
...
public:
	...
	Hit intersect(Ray ray)
	{
		...
		Ray *localRay = new Ray(ray.origin, ray.direction);
...
public:
	...
	Hit intersect(Ray ray)
	{
		...
		///< Local height of the cone
		float height = 1.0;
		///< Local height of the cone
    	glm::vec3 H = glm::vec3(0.0f, height, 0.0f); 
		///< origin of the cone
		...
		localRay->origin = glm::vec3(inverseTransformationMatrix * glm::vec4(ray.origin, 1.0f ));
		localRay->direction = glm::normalize(glm::vec3(inverseTransformationMatrix * glm::vec4(ray.direction, 0.0f)));
		...
		*/
		...
		// plane
        Plane base = Plane(material, helperForTR(glm::vec3(0),glm::vec3(180.0, 0, 0),glm::vec3(0.5f)));
        ...
        if (baseHit.hit && baseHit.distance < closest_t || (hit.intersection.y < 0 || hit.intersection.y > height)) {
            ...
            if (distance <= radius) {
				...
				baseHit.normal = glm::normalize(glm::vec3(normalMatrix * glm::vec4(baseHit.normal, 0.0f)));
				baseHit.intersection = glm::vec3(transformationMatrix * glm::vec4(baseHit.intersection, 1.0f));
				baseHit.distance = glm::length(baseHit.intersection - ray.origin);
...
void sceneDefinition()
{
	...
	*/
	...
	// using the helper function, apply the coordinates to both cones
	Cone *cone1 = new Cone(yellow);
	glm::mat4 cone1T = helperForTR(glm::vec3(5.0, -3.0, 14.0),glm::vec3(0.0),glm::vec3(3.0f, 12.0f, 3.0f));
	cone1->setTransformation(cone1T);
	objects.push_back(cone1);
	
	Cone *cone2 = new Cone(green_diffuse);
	glm::mat4 cone2T = helperForTR(glm::vec3(3.5, -2.0, 8.0),glm::vec3(0.0, 0.0, -110.0),glm::vec3(1.0f, 3.0f, 1.0f));
	cone2->setTransformation(cone2T);
	objects.push_back(cone2);
