...
public:
	...
	Hit intersect(Ray ray){
		...
		float ray_t = glm::dot(point - ray.origin, normal) / glm::dot(ray.direction, normal);

		if (ray_t > 0) {
			hit.hit = true;
			hit.intersection = ray.origin + ray.direction * ray_t;
			hit.distance = glm::distance(ray.origin, hit.intersection);
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material){
	...
	for(int light_num = 0; light_num < lights.size(); light_num++){
		...
		*/
		...
		// If there is a texture, take the diffuse color of the texture instead
		...
		float distance = glm::distance(point, lights[light_num]->position);
		float attenuation = 1/ glm::pow(distance, 2);

		color += attenuation * lights[light_num]->color * (diffuse_color*diffuse + material.specular*specular);

		// color += lights[light_num]->color * (diffuse_color*diffuse + material.specular*specular);
	}
...
void sceneDefinition (){
    ...
    green_diffuse.diffuse = glm::vec3(0.7f, 1.0f, 0.7f);
    green_diffuse.specular = glm::vec3(0.0);
    green_diffuse.shininess = 0.0;
	...
	objects.push_back(new Sphere(5.0, glm::vec3(7,3,23), checkerboard));

	Material rainbow;
	rainbow.texture = &rainbowTexture;
	rainbow.specular = glm::vec3(1.0);
	rainbow.shininess = 10.0;
	objects.push_back(new Sphere(7.0, glm::vec3(-6,4,23), rainbow));
	...
	*/
	...
	// Lights
	lights.push_back(new Light(glm::vec3(0, 26, 5), glm::vec3(20.0)));
	lights.push_back(new Light(glm::vec3(0, 1, 12), glm::vec3(40.0)));
	lights.push_back(new Light(glm::vec3(0, 5, 1), glm::vec3(60.0)));
