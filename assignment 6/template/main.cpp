...
// Saw an implemenentation in class where a helper function for Transformation, Rotation and Scaling for matrices was used and chose to also implement (will re-use for future assignments)
glm::mat4 helperForTR(glm::vec3 t, glm::vec3 r, glm::vec3 s) {

        glm::mat4 translated = glm::translate(glm::mat4(1.0f), t);
        glm::mat4 rotated = glm::rotate(translated,glm::radians(r.x), glm::vec3(1,0,0));

        rotated = glm::rotate(rotated, glm::radians(r.z), glm::vec3(0,0,1));
        rotated = glm::rotate(rotated, glm::radians(r.y), glm::vec3(0,1,0));


        glm::mat4 scaled = glm::scale(rotated, s);
        return scaled;
}
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material){
	...
	for(int light_num = 0; light_num < lights.size(); light_num++){
		...
		Ray shadow_ray(point + normal * 0.001f, light_direction); // the shadow ray is defined as
		
		bool bool_shadow_hit = false; // initialized as false

		for(int j = 0; j < objects.size(); j++) {
			Hit shadow_hit = objects[j]->intersect(shadow_ray);

			if(shadow_hit.distance < glm::distance(point, lights[light_num]->position) && shadow_hit.distance > 0 && shadow_hit.hit == true) {
					bool_shadow_hit = true;
					goto end;
			}
		}
		end: ;
...
/**
 * Function that calculates the Fresnel Effect
 * @param cos the cos(theta of I)
 * @param n1 reflection coefficient
 * @param n2 refraction coefficient
 * @return float incident reflectivity
 */
float fresnelReflection(float cos, float n1, float n2){
	// according to the formula
	float r_0 = pow(((n1 - n2) / (n1 + n2)), 2);
	return r_0 + (1.0f - r_0) * pow(1.0f - cos, 5);
...
/**
 ...
 @param depth depth cue information (used as stopping condition)
...
glm::vec3 trace_ray(Ray ray, int depth){
	...
	if(closest_hit.hit){
		...
		glm::vec3 ray_d_normalized = glm::normalize(-ray.direction); 
		...
		if(material.reflection_level > 0) {
			...
			color = color * (1.0f - material.reflection_level) + (reflection_color * material.reflection_level);
		}

		if(material.refractive_level > 0) {
			float cos_ti = glm::dot(ray_d_normalized, closest_hit.normal);
			...
			if(inside) {
				...
				n1 = material.refractive_level;
			}

			float n2;
			if(inside) {
				n2 = material.refractive_level;
			} else {
				n2 = 1.0f;
			}
			...
			if(inside) {
				refraction_direction = glm::refract(ray.direction, closest_hit.normal, refractive_index_ratio);
			} else {
				refraction_direction = glm::refract(ray.direction, -closest_hit.normal, refractive_index_ratio);
			}

			Ray refraction_ray(closest_hit.intersection + 0.001f * refraction_direction, refraction_direction);

			// recursive (whitted-style)
			glm::vec3 refraction = trace_ray(refraction_ray, depth - 1);

			float cos_t = abs(cos_ti);

			float reflectivity = fresnelReflection(cos_t, n1, n2);
			float refractivity = 1.0f - reflectivity;

			color = color * reflectivity + refractivity * refraction;
		}
	} else {
...
void sceneDefinition (){
	...
	blue_specular.reflection_level = 1.0f;

	Material refractive_material;
	refractive_material.refractive_level = 2.0f;
	refractive_material.reflection_level = 1.0f;
	...
	glm::mat4 scalingMatrix = glm::scale(glm::vec3(3.0f, 5.0f, 3.0f));
...
int main(int argc, const char * argv[]) {
    ...
    for(int i = 0; i < width ; i++)
        for(int j = 0; j < height ; j++){
			...
			// depth starting at 4
			image.setPixel(i, j, toneMapping(trace_ray(ray, 4)));
